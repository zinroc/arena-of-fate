    ...
    return {
        ...
        */
        ...
        create: function (name, email) {
            ...
            .then(function (r) {
                //create starting rooms with index 0,1,2,3
                id = r[0];
                var promises = [];
                var room_names = ['strategy_center', 'empty', 'empty', 'empty'];

                for (var i=0; i<4; i++){
                    var room = {name: room_names[i], owner: email, index: i};
                    promises[i] = knex("player_rooms").insert(room);
                }
