...
app.factory("gameAPIservice", function gameAPIservice ($http) {
    ...
    this.updateWinner = function (winner_id, loser_id, match_id, email){
        return this.postJSON("/api/matches/" + match_id + "/record",
    ...
    this.recordPlayerStrategy = function (fight_round, strat_id, match_id, email){
        return this.postJSON("/api/matches/" + match_id + "/recordStrat", 
            {fight_round: fight_round, strat_id: strat_id, email: email});
    };
    ...
    this.getMatch = function (email, id){
        return this.getJSON("/api/challenges/" + id, { email: email });
    };

    this.getArena = function (email, id){
        return this.getJSON("/api/challenges/arena/" + id, {email: email});
    };
...
app.controller('fightPlannerController', function ($scope, gameAPIservice, $timeout) {
    "use strict";

    $scope.email = utils.getCookie("email");
    $scope.player_name = utils.getCookie("name");
    $scope.matchID = utils.getURLParams().match_id;
    console.log($scope.matchID, "MATCH ID");
...
/**----------------------- INIT  -------------------**/
    ...
    $scope.loaded.arenaFighter = false;
    $scope.loaded.playerFighter = false;
    ...
    gameAPIservice.getSkills().then(function (response){
        ...
        if (response.data.hasOwnProperty('status') && response.data.status === 'error') {
            ...
            $scope.skillIDs = response.data.skillIDs;
        }
    });
    ...
    gameAPIservice.getMatch($scope.email, $scope.matchID).then(function (response){
        console.log("Tried to fetch match");
        console.log(response.data);
        ...
        if (response.data.hasOwnProperty('status') && response.data.status === 'error') {
            ...
            console.log("Setting random seed to " + $scope.match.random_seed);
            $scope.random_seed = $scope.match.random_seed;

        }
    }).then(function (){
        return gameAPIservice.getArena($scope.email, $scope.match.arena).then(function (response){
            console.log("Tried to fetch arena");
            console.log(response.data);
            if (response.data.hasOwnProperty('status') && response.data.status === 'error') {
                $scope.arena = null;
            } else {
        ...
        return gameAPIservice.getFighter($scope.email, $scope.match.player_fighter).then(function (response){
            console.log("Tried to fetch player fighter");
            console.log(response.data);

            if (response.data.hasOwnProperty('status') && response.data.status === 'error') {
                $scope.player_fighter = null;
            } else {
                $scope.player_fighter = response.data;
                return $scope.placeInCorner($scope.player_fighter.id, 'red');
            }
        });
    }).then(function (){
        return gameAPIservice.getFighter($scope.email, $scope.match.arena_fighter)
        .then(function (response){
            console.log("Tried to fetch arena fighter");
            console.log(response.data);

            if (response.data.hasOwnProperty('status') && response.data.status === 'error') {
                $scope.arena_fighter = null;
            } else {
                $scope.arena_fighter = response.data;
                return $scope.placeInCorner($scope.arena_fighter.id, 'blue');            }
        });
    }).then(function(){
        $scope.checkIfRecording();
    });
//place selected fighter on the fighter list to a corner 
    ...
    $scope.placeInCorner = function (id, side){
        ...
        if (side==='red'){
            $scope.corner[side] = $scope.player_fighter;
            $scope.experience[side] = $scope.player_fighter.experience;

        } else if (side==='blue') {
            $scope.corner[side] = $scope.arena_fighter;
            $scope.experience[side] = $scope.arena_fighter.experience;
        } else {
            console.log("incorrect side init");
        }

        $scope.corner[side].status = 'art';

        return gameAPIservice.getPlans($scope.corner[side].id).then(function (response){
            console.log("Tried to fetch strategies");
            console.log(response.data);
            ...
            if (response.data.hasOwnProperty('status') && response.data.status === 'error') {
                ...
                $scope.corner[side].plans = response.data.plans;
                $scope.corner[side].selectedPlan = response.data.plans[0];
            }
        }).then(function (){
            return gameAPIservice.getSlots($scope.corner[side].id).then(function (response){
                ...
                if (response.data.hasOwnProperty('status') && response.data.status === 'error') {
                    ...
                    $scope.corner[side].slots = response.data.slots;
                }


            });
        }).then(function (){
            return gameAPIservice.getTechCond($scope.corner[side].id).then(function (response){
                ...
                if (response.data.hasOwnProperty('status') && response.data.status === 'error') {
                    ...
                    $scope.corner[side].techCond = response.data.techConditioning;
                }
                if (side==='red'){
                    $scope.loaded.playerFighter = true;
                } else if (side==='blue'){
                    $scope.loaded.arenaFighter = true;
                }
            ...
            return $scope.initializeCornerStrategy(side);
        });
    ...
    $scope.showPrizePool = function (){
        $('#prizeModal').modal('show');
    };

    $scope.checkIfRecording = function (){
        if ($scope.loaded.arenaFighter && $scope.loaded.playerFighter && $scope.corner['blue'].selected && $scope.corner['red'].selected){
            if ($scope.match.round_1_strat){
                $scope.recording = true;
                console.log($scope.corner, $scope.corner['red'], $scope.corner['red'].selectedPlan);
                var planIndex = $scope.getIndexOf($scope.corner['red'].plans, "strategy", $scope.match.round_1_strat);
                $scope.selectPlan('red', $scope.corner['red'].plans[planIndex]);
                $scope.evaluateFight();
            } else {
                $scope.recording = false;
                //$scope.evaluateFight();
    ...
    $scope.myRandom = function (){
        Math.seedrandom($scope.random_seed);
    ...
    $scope.checkIfRecordingRound = function (round){
        var index = "round_"+round+"_strat";
        //    console.log("checking recording " + round, $scope.match[index], $scope.match);
        if ($scope.match[index]){
            $scope.recording = true;
            console.log("continue to round " + round);

            var planIndex = $scope.getIndexOf($scope.corner['red'].plans, "strategy", $scope.match[index]);
            $scope.selectPlan('red', $scope.corner['red'].plans[planIndex]);
    ...
    $scope.random_seed = null;

/**--------------------------------- INIT END---------------------------------**/
/**--------------------------------- Indexers ---------------------------------**/

    $scope.getAttendance = function (prize_pool, ticket_price){
        var attendance = Math.round(prize_pool/ticket_price);
        return attendance;
    };

    $scope.getAttendancePercent = function(prize_pool, ticket_price, capacity){
        var attendance = $scope.getAttendance(prize_pool, ticket_price);
        var percent = Math.round((attendance*100)/capacity);
        return percent;
    };

    $scope.selectFiPara = function (param, side){
        console.log(param);
        $scope.selectedFiPara = {};
        $scope.selectedFiPara.name = param; 
        var level = $scope.getStratParamHeight(side, param);
    ...
    $scope.heightConversion = function (height){
        var rand = parseInt(33*$scope.myRandom());
        if (height==='low'){
            return rand;
    ...
    $scope.getIndexOf = function(array, object, target){
      //  console.log(array, object, target);
        var result = false;
    ...
    $scope.getStratParamHeight = function (side, param){
        if(typeof ($scope.corner[side].selectedPlan) !=='undefined'){
            if(typeof($scope.strats[$scope.corner[side].selectedPlan.strategy][param]) !=='undefined'){
                return $scope.strats[$scope.corner[side].selectedPlan.strategy][param];
            } else {
                return false;
            }
        }
...
/**--------------------------------- Fight Functions--------------------------------- **/
    ...
    $scope.recordPlayerStrategy = function(){
        console.log($scope.fight.round, $scope.strat['red'].id, $scope.matchID, "PLAYER STRAT");
    
        gameAPIservice.recordPlayerStrategy($scope.fight.round, $scope.strat['red'].id, $scope.matchID, $scope.email)
        .then(function (response){
            console.log("Tried to update player strategy for round " + $scope.fight.round);
            console.log(response.data);

            if(response.data.hasOwnProperty('status') && response.data.status ==='error') {
                console.log('error');
            } else {
                console.log("round strategy recorded");
            }
        });
    };
    ...
    $scope.updateWinner = function (){
        ...
        .then(function (response){
            ...
            if (response.data.hasOwnProperty('status') && response.data.status === 'error') {
                ...
                $scope.loaded.updateRecords = true;
                console.log("fight records updated");
            }
        });
    };
    ...
    $scope.checkPostInitTechniques = function (initiatior, defender){
    ...
    $scope.conditionCheck = function (side, tech, exp){
        ...
        exp = parseInt(exp);
        var bar = parseInt(parseInt(tech.technical_value)*33*$scope.myRandom()) + 33*$scope.myRandom();

        var score = parseInt($scope.myRandom()*exp);

        var msg ="";
        if (tech.name==='mezmorizing_gaze'){ // improve with poison

            bar += $scope.myRandom()*33;

            var multiplier = $scope.poisonMultiplier($scope.checklist[otherSide].poison);
            if (multiplier > 0){
                score += multiplier*33*$scope.myRandom();
    ...
    $scope.checkTechActive = function (side, name){
       for(var i=0; i<$scope.allSlots.length; i++){
            var techID = $scope.activeTechniques[side][$scope.allSlots[i]].id;
            if (techID){
                var tech = $scope.getTech(techID);
                if(tech.name===name){
                    tech.exp = $scope.activeTechniques[side][$scope.allSlots[i]].exp;
                    return tech;
                }
            } else {
                //do noting, itterating through empty slot
            }
        }
        return false;
    };
    ...
    $scope.checkTechSewingMachine = function(side){
        ...
        if (tech){
            ...
            $scope.record("blocks: " + $scope.checklist[side].blocks);
    ...
    $scope.resolveSewingMachine = function(side){
        $scope.record("inside sewing_machine "+ side);
        var otherSide = $scope.otherSide(side);
        var msg = "";
        var time = "";
    ...
    $scope.checkTechFlying = function(side){
        for(var i=0; i<$scope.allSlots.length; i++){
            ...
            if (techID){
                ...
                //do nothing, itterating throug empty slot
            }
        }
        return false;
    };
    ...
    **/
    ...
    $scope.fightpositioning = function(side){
        ...
        var regen = parseInt($scope.myRandom()*skill - $scope.myRandom()*opposingSkill);
    ...
    */
    $scope.addInjury = function(side, level){
        ...
        // pick an injury
        if (level === 1){
            injuryIndex = Math.floor($scope.myRandom()*($scope.injuries[levelIndex].length));
            ...
            if(j===0){
                ...
                var upgradeIndex = Math.floor($scope.myRandom()*j);
    ...
    $scope.stallForCardio = function (side){
        ...
        var regen = parseInt($scope.myRandom()*base*injuryMod);
    ...
    $scope.fightConsciousness = function (){
        for (var i=0; i<$scope.sides.length; i++){
            ...
            if($scope.corner[side].dazed){
                ...
                $scope.vitals[side].consciousness += parseInt(($scope.myRandom()*skill)/10);
            }
        }
    };
    ...
    $scope.getPowerScore = function (side){
        ...
        return parseInt(base*$scope.myRandom());
    };
    ...
    $scope.getSpeedScore = function (side){
        ...
        return parseInt(base*$scope.myRandom()*injuryMod);
    };
    ...
    $scope.checkAdvantage = function (initiator, defender){
        ...
        //$scope.corner[initiator].experience[$scope.strat[initiator].id].as;
        var initiatorAs = $scope.corner[initiator].experience[$scope.strat[initiator].id].as;
        var initiatorAgainst = $scope.corner[initiator].experience[$scope.strat[initiator].id].against;
        var defenderAs = $scope.corner[defender].experience[$scope.strat[initiator].id].as;
        var defenderAgainst = $scope.corner[defender].experience[$scope.strat[initiator].id].against;
    ...
    $scope.resolveEvasiveStrikes = function(side){
        ...
        var msg = $scope.corner[side].name.toTitleCase() + " performs an evasive strike and evades the counter strike!";
        $scope.record(msg);
    ...
    $scope.evaluateBlock = function (blocker, reflex, speed, power){
        ...
        if (blockScore < power){
            if ($scope.activeTraits[blocker].name==='holy' && $scope.checklist[blocker].holyShield > 0){
                ...
                msg = $scope.corner[blocker].name.toTitleCase() + "'s divine shield adsorbes " + reduction + 
                " damage";
                $scope.record(msg);
    ...
    $scope.resolveAdvancingWall = function (side){
        ...
        var positioningDMG = parseInt($scope.myRandom()*20 + 10); 
    ...
    $scope.resetTempCombatMods = function (){
        for(var i=0; i<$scope.sides.length; i++){
            ...
            $scope.checklist[$scope.sides[i]].holyTech = false;
    ...
    */
    $scope.regenConsciousness = function(side){
        ...
        base = parseInt(base*$scope.myRandom()*injuryMod);
    ...
    $scope.regenCardio = function(side){
        ...
        var regen = parseInt(base*$scope.myRandom());
    ...
    $scope.checkVulnerable = function(side){
        ...
        var vulnerability = 100*$scope.myRandom()*$scope.myRandom();
        if ($scope.fumble[side]){

            vulnerability += 50*$scope.myRandom();
        }
        if ($scope.corner[side].gassed){
    ...
    $scope.getpositioningDamage = function (target, damage, vulnerableMod){
        ...
        var skillMod =  ($scope.myRandom()*attackerSkill - $scope.myRandom()*targetSkill)/100;
    ...
    $scope.getBloodiedDamage = function(target, damage){
        ...
        var skillMod = (attackerSkill * $scope.myRandom())/80;
    ...
    $scope.checkQuit = function (target){
        ...
        if($scope.myRandom()*base > $scope.myRandom()*skill){
    ...
    /**
    * plan is an entry in the array recieved from getPlans
    * Use this to select a fighters plan
    */
    ...
    $scope.initializeCornerStrategy = function (side){
        console.log("STARTING CORNER STRAT INIT FOR ", side, $scope.corner[side].selectedPlan.strategy);
        ...
        for(var k=0; k<$scope.allSlots.length; k++){
            ...
            //console.log(side, $scope.strat, "STRAT <---");
            //anchor
            var techID = $scope.corner[side].slots[$scope.strat[side].id][$scope.allSlots[k]];

            $scope.activeTechniques[side][$scope.allSlots[k]].id = techID;
            if (techID){
                ...
                $scope.activeTechniques[side][$scope.allSlots[k]].exp = 0;
                $scope.activeTechniques[side][$scope.allSlots[k]].name = "Empty";
                $scope.activeTechniques[side][$scope.allSlots[k]].range = "NA";
                $scope.activeTechniques[side][$scope.allSlots[k]].cardio_cost = 0;
                $scope.activeTechniques[side][$scope.allSlots[k]].technical_value = 0;
            }
        }
        var j=0;
        //obj.hasOwnProperty(prop)
        for (var i=0; i<$scope.skills.length; i++){
            //console.log($scope.stratBonuses, $scope.corner[side].strategy, "STRAT BONUSES");
            if ($scope.stratBonuses[$scope.corner[side].strategy].hasOwnProperty($scope.skills[i].id)){
            ...
            //initialize Skills
            if (side==='red'){
                $scope.cSkills[side][$scope.skills[i].name] = $scope.player_fighter.skills[$scope.skills[i].id];
            } else if (side==='blue') {
                $scope.cSkills[side][$scope.skills[i].name] = $scope.arena_fighter.skills[$scope.skills[i].id];

            } else {
                console.log("bad side init");
                return;
            }
        }
...
/**--------------------------------- Dynamic UI --------------------------------- **/
    ...
    $scope.doTechChecks = function (side, tech){
        ...
        if(tech.cardio_cost!==0 && tech.name!=='turtle') {
            ...
            //console.log(reqCardio, "reqCardio");
        ...
        if (tech.name==='sewing_machine'){
            ...
            var blocksNeeded = 3 - $scope.checklist[side].blocks; 
            if (blocksNeeded < 0){
                blocksNeeded = 0;
            }

            msg += blocksNeeded + " more subsequent block <br>";
        }
        //initiator
    ...
    $scope.selectTrait = function(trait){
        $scope.selectedTrait = $scope.traits[trait];
    };
    ...
    $scope.endAnimations = function (){
        $scope.endAnimationsGeneral();

        if (!$scope.fight.stopped && $scope.fight.paused){
            $scope.checkIfRecordingRound($scope.fight.round+1);
        }
        $scope.fightLoop();
    };
